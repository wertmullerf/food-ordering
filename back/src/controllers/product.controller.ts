...
export const obtenerProductos = async (req: Request, res: Response) => {
  try {
    ...
    if (reply) {
      res.json(reply.productos ?? []);
      return;
    }

    const productos = await Producto.find().populate("ingredientes.id").lean();
    ...
    await saveResult({ productos: productosFormateados }, "product");
...
export const agregarProducto = async (
  ...
  try {
    ...
    const { nombre, precio, stock, costo, descripcion, categoria } = req.body;
